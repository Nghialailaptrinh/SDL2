...
bool loadMedia()
{
	...
	SDL_RWops* file = SDL_RWFromFile( "save_game/nums.bin", "r+b" );
	...
	if( file == NULL )
	{
		...
		//Create file for writins
		file = SDL_RWFromFile( "save_game/nums.bin", "w+b" );
		if( file != NULL )
		{
...
void close()
{
	...
	SDL_RWops* file = SDL_RWFromFile( "save_game/nums.bin", "w+b" );
	if( file != NULL )
	{
		//Save data
		for( int i = 0; i < TOTAL_DATA; ++i )
